...
def extract_stat_with_american_odds(input_file, output_file):
    ...
    # Ensure the 'nfl/lines' folder exists.
    os.makedirs('nfl/lines', exist_ok=True)
    
    # Write the output file inside the 'nfl/lines' folder.
    with open(f'nfl/lines/{output_file}', 'w') as f:
        json.dump(output_list, f, indent=2, ensure_ascii=False)
    
    print(f"Extracted data for {len(output_list)} players from '{input_file}' to 'nfl/lines/{output_file}'")


# List of all NFL stat types to process.
nfl_stat_types = [
    "passing_yards",
    "rushing_yards",
    "receiving_yards",
...
# Automatically run the extraction for each NFL stat file when this module is imported.
for stat in nfl_stat_types:
    input_filename = f"nfl/data/{stat}.json"
